...
- `_extract_meta()` 로 year/month/min_rank/rank/min_likes/min_play/max_play/sort/kind/playable 를 정규식 추출해 `search_videos` args 에 주입(LLM 누락 방어). 평점 "N 이상"→`min_rank`, "랭크 N"/"별점 N"→`rank`, "좋아요/하트/찜 N"→`min_likes`, "재생 N 이상/이하"→`min_play`/`max_play`, "최근/마지막에 본"→`sort=recent`, "오래 안 본"→`sort=oldest`, **"아무거나/랜덤/무작위/임의로/되는대로"→`sort=random`**, **"인기/많이 본/자주 본/베스트/TOP/핫한/좋아요 많은/평점 높은"→`sort=popular`**(우선순위 random > popular > recent > oldest). studio/actress 는 query 로 semantic+FTS 매칭.
