...
import $ from 'jquery';
import { LocalStorageItem, Random, PATH, Popup } from '../crazy.common.js';
...
var displayGrid = function () {
  if (tile && !neon) {
    ...
    $('#tileGrid')
      .empty()
      .toggleClass('grid-tile', !rotate)
      .append(
        (function () {
          var $tbody = $('<tbody>');
          for (var i = 0; i < TILE.row; i++) {
            var $tr = $('<tr>')
              .height(window.innerHeight / TILE.row)
              .appendTo($tbody);
            for (var j = 0; j < TILE.col; j++) {
              $('<td>', { class: rotate ? '' : 'grid-tile' }).appendTo($tr);
            }
          }
          return $tbody;
        })()
