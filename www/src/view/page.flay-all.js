...
// 데이터 로드 이벤트 리스너
flayAll.addEventListener('data-loaded', (e) => {
...
// 페이지 변경 이벤트 리스너
flayAll.addEventListener('page-changed', (e) => {
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.ctrlKey && e.key === 'f') {
    e.preventDefault();
    const searchInput = flayAll.shadowRoot.querySelector('#search-input');
    searchInput.focus();
  }

  // 이전/다음 페이지: ←/→ 화살표 키
  if (e.key === 'ArrowLeft') {
    const prevButton = flayAll.shadowRoot.querySelector('#prev-page');
    if (!prevButton.disabled) {
      prevButton.click();
    }
  } else if (e.key === 'ArrowRight') {
    const nextButton = flayAll.shadowRoot.querySelector('#next-page');
