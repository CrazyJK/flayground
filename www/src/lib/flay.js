...
export default class Flay {
  ...
  static fileLength(bytes) {
    return bytes < Flay.MB ? (bytes / Flay.KB).toFixed(0) + ' <small>KB</small>' : bytes < Flay.GB ? (bytes / Flay.MB).toFixed(0) + ' <small>MB</small>' : (bytes / Flay.GB).toFixed(1) + ' <small>GB</small>';
  }
  ...
  $() {
    ...
    return $('<section>', { class: 'flay ' + (this.archive ? 'flay-archive' : 'flay-instance') }).append(
      ...
      $('<dl>', { class: 'flay-body ' + (Flay.separateMode ? 'flay-body-separate' : '') }).append(
        ...
        $('<dd>', { class: 'flay-studio' }).append($('<div>').append(this.studio)),
        $('<dd>', { class: 'flay-opus' }).append($('<div>').append(this.opus)),
        ...
        $('<dd>', { class: 'flay-release' }).append($('<div>').append(this.release)),
            ...
            .append(
              (() => {
                ...
                Flay.RANKs.forEach((r) => {
                  ret.push($('<label>').append($('<input>', { type: 'radio', name: 'rank', value: r, checked: r === this.video.rank }), $('<i>', { class: 'hover fa fa-' + (r === -1 ? 'thumbs-down' : r === 0 ? 'circle' : 'star') + ' r' + r })));
                });
                return ret;
              })()
            )
        ...
        $('<dd>', { class: 'flay-info' }).append(
          $('<div>').append(
            ...
            $('<span>', { class: 'extra flay-play hover' })
              .html(this.video.play + ' <small>play</small>')
            ...
            $('<span>', { class: 'extra flay-view hover' })
              .html(this.files.movie.length > 0 ? '<small>View</small>' : '')
              .on('click', this, Flaying.start),
        ...
        $('<dd>', { class: 'flay-comment' }).append(
          $('<div>').append(
            $('<input>', { class: 'extra comment-edit hover' })
        ...
        $('<dd>', { class: 'flay-modified' }).append($('<div>').append($('<span>', { title: 'last modified' }).append(Flay.date(this.lastModified)), $('<small>', { class: 'extra', title: 'last modified' }).append(Flay.time(this.lastModified)))),
          ...
          $('<div>').append(
            (() => {
              ...
              Flay.tagList.forEach((t) => {
                ret.push($('<span>', { class: 'tag-item' + (this.video.tags.indexOf(t.id) > -1 ? ' active' : ''), title: t.description, id: 'tag' + t.id }).html(t.name));
              });
              return ret;
            })()
          )
        ...
        $('<dd>', { class: 'flay-files' }).append(
          (() => {
            ...
            this.files.cover.forEach((f) => {
              ret.push($('<div>', { class: 'flay-file-cover' }).html(f));
            });
            this.files.movie.forEach((f) => {
              ret.push($('<div>', { class: 'flay-file-movie' }).html(f));
            });
            this.files.subtitles.forEach((f) => {
              ret.push($('<div>', { class: 'flay-file-sub' }).html(f));
            });
            this.files.candidate.forEach((f) => {
              ret.push($('<div>', { class: 'flay-file-candi' }).html(f));
            });
            return ret;
          })()
        )
      ...
      $('<div>', { class: 'right-bottom' }).append(
        $('<span>', { class: 'flay-body-toggle hover' })
          .append($('<i>', { class: 'fa ' + (Flay.separateMode ? 'fa-arrow-circle-up' : 'fa-arrow-circle-down') }))
          .on('click', () => {
      ...
      $('<div>', { class: 'flay-video' }).append($('<video>'), $('<div>').append($('<i>', { class: 'hover fa fa-window-restore' }).on('click', this, Flaying.window.restore), $('<i>', { class: 'hover fa fa-window-maximize' }).on('click', this, Flaying.window.maximize), $('<i>', { class: 'hover fa fa-window-close' }).on('click', this, Flaying.stop)))
    );
  }
}
...
var Flaying = {
  ...
  start: function (e) {
    ...
    if (!Flaying.isPlay) {
      ...
      $video.off('wheel').on('wheel', e.data, function (e) {
        e.stopPropagation();
