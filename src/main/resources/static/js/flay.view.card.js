...
(function ($) {
    $.fn.appendFlayCard = function (flay, args) {
        ...
        if (typeof SockJS === "undefined") {
            $("head").append('<script type="text/javascript" src="/js/flay.websocket.js"></script>');
        }
        ...
        var getRank = function (opus) {
                return (
                    '<span class="ranker flay-rank">' +
                    '<label><input type="radio" name="flay-rank-' +
                    opus +
                    '" value="-1"><i class="fa fa-thumbs-down r-1"></i></label>' +
                    '<label><input type="radio" name="flay-rank-' +
                    opus +
                    '" value="0"><i class="fa fa-circle r0"></i></label>' +
                    '<label><input type="radio" name="flay-rank-' +
                    opus +
                    '" value="1"><i class="fa fa-star r1"></i></label>' +
                    '<label><input type="radio" name="flay-rank-' +
                    opus +
                    '" value="2"><i class="fa fa-star r2"></i></label>' +
                    '<label><input type="radio" name="flay-rank-' +
                    opus +
                    '" value="3"><i class="fa fa-star r3"></i></label>' +
                    '<label><input type="radio" name="flay-rank-' +
                    opus +
                    '" value="4"><i class="fa fa-star r4"></i></label>' +
                    '<label><input type="radio" name="flay-rank-' +
                    opus +
                    '" value="5"><i class="fa fa-star r5"></i></label>' +
                    "</span>"
                );
            },
        ...
        var constructFlay = function () {
            ...
            if (settings.exclude.includes(ACTION)) {
                ...
                $flayCard
                    .find(".flay-subtitles")
                    .toggle(flay.files.subtitles.length > 0)
                    .on("click", function () {
        ...
        var constructActress = function ($wrapper) {
            var setFavorite = function ($actress, actress) {
                if (actress.favorite) $actress.find(".flay-actress-favorite > i").addClass("fa-star favorite").removeClass("fa-star-o");
                else $actress.find(".flay-actress-favorite > i").addClass("fa-star-o").removeClass("fa-star favorite");
            };
