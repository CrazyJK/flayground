...
public class SecurityConfig {
  ...
  protected SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
    http.headers().frameOptions().sameOrigin()
        ...
        .requestMatchers("/", "/favicon.ico", "/index.html", "/webjars/**", "/flayground-websocket/**", "/dist/**").permitAll()
        .requestMatchers("/batch/**").hasRole("ADMIN")
        .anyRequest().authenticated()
        .and()
        .formLogin().loginPage("/dist/login.html").permitAll().successHandler(getFlayAuthenticationSuccessHandler()).failureHandler(getFlayAuthenticationFailureHandler())
        .and()
