...
public abstract class FlayServiceAdapter {
  ...
  protected Comparator<Flay> releaseReversedComparator = Comparator.comparing(Flay::getRelease).reversed();
  ...
  protected Collection<Flay> findByField(Collection<Flay> list, String key, String value) {
    if ("studio".equalsIgnoreCase(key)) {
      return list.stream().filter(f -> f.getStudio().equals(value)).sorted(releaseReversedComparator).toList();
    } else if ("title".equalsIgnoreCase(key)) {
      return list.stream().filter(f -> f.getTitle().contains(value)).sorted(releaseReversedComparator).toList();
    } else if ("actress".equalsIgnoreCase(key)) {
      return list.stream().filter(f -> f.getActressList().stream().anyMatch(a -> a.equals(value))).sorted(releaseReversedComparator).toList();
    } else if ("release".equalsIgnoreCase(key)) {
      return list.stream().filter(f -> f.getRelease().startsWith(value)).sorted(releaseReversedComparator).toList();
    } else if ("rank".equalsIgnoreCase(key)) {
      final int rank = Integer.parseInt(value);
      return list.stream().filter(f -> f.getVideo().getRank() == rank).sorted(releaseReversedComparator).toList();
    } else if ("play".equalsIgnoreCase(key)) {
      final int play = Integer.parseInt(value);
      return list.stream().filter(f -> f.getVideo().getPlay() == play).sorted(releaseReversedComparator).toList();
    } else if ("comment".equalsIgnoreCase(key)) {
      return list.stream().filter(f -> f.getVideo().getComment().contains(value)).sorted(releaseReversedComparator).toList();
    } else if ("tag".equalsIgnoreCase(key)) {
      final int id = Integer.parseInt(value);
      return list.stream().filter(f -> f.getVideo().getTags().stream().anyMatch(t -> t.getId().intValue() == id)).sorted(releaseReversedComparator).toList();
    } else {
