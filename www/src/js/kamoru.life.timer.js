...
(function ($) {
	$.fn.lifeTimer = function (options) {
		...
		var HTML = `<div id="life-timer" style="display: none;">
		...
		var startTimer = function () {
			...
			if (!opts.progress) $progress.hide();
			else if (opts.progress === 'bottom') $progress.insertAfter('#life-timer .display-time');
			...
			var fnTimer = function () {
				...
				$displayTime.html(
					opts.pattern.replace(/(day|hh|mm|ss)/g, function ($1) {
						switch ($1) {
							case 'day':
			...
			if (opts.onlyOnce) fnTimer();
			else timer = setInterval(fnTimer, 1000);
		};
