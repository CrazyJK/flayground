...
function ProgressBar(selector, args) {
	...
	$("<div>", {id: this.selector}).addClass("progress w-100 rounded-0").css({
		height: settings.height,
		backgroundColor: settings.backgroundColor
	}).append(this.$bar).appendTo($(settings.wrap).addClass(settings.wrapClass));
...
function PlayBar(selector) {
	...
	this.$bar1 = $("<div>", {class: 'progress-bar bg-dark'}).css({
		width: 0,
		transition: 'width .5s linear'
	});
	this.$bar2 = $("<div>", {class: 'progress-bar bg-warning'}).css("width", "0");
	this.$bar  = $(selector).addClass("progress bg-dark").css({
		width: '100%',
		height: '1px',
		margin: '0 4px'
	}).append(
...
function FlayImageEngine(args) {
	...
	if (_self.settings.showControlBox) {
		function getSwitch(id, key, func, val) {
			return $("<div>", {class: 'custom-control custom-switch'}).append(
					$("<input>", {type: 'checkbox', class: 'custom-control-input', id: id}).on("change", func).prop('checked', val),
					$("<label>", {class: 'custom-control-label text-capitalize', for: id, title: id + ' ' + key}).html(id)
			);
		}
		...
		if (_self.settings.imageInfo.length > 0) {
			$("<div>", {id: 'infoBox', class: "text-light small py-1"}).css({
				position: 'fixed',
		    	bottom: 0,
		    	right: 0,
				fontFamily: 'D2Coding',
		...
		$("<div>", {id: 'controlBox', class: 'text-light small p-1'}).css({
			position: 'fixed',
	    	bottom: 0,
	    	right: 0,
			fontFamily: 'D2Coding',
	    	...
	    	borderTopLeftRadius: '4px'
				...
				$("<ul>", {class: 'list-unstyled text-center m-0'}).append(
						$("<li>").append(
								getSwitch('random', _self.settings.key.random, function() {
						...
						$("<li>", {class: 'mb-1'}).append(
								$("<div>", {class: 'rounded'}).css({
							    	...
							    	flexWrap: 'wrap'
										...
										$("<input>", {type: 'number', min: _self.minPlayInterval, id: 'playInterval', class: 'bg-transparent border-0 text-light text-right'}).css({
											flex: '1 1 auto',
											width: 0
										}).val(_self.playIntervalTime).on("change", function() {
						...
						$("<li>", {class: 'mb-1'}).append(
								$("<div>", {class: 'rounded'}).css({
							    	...
							    	flexWrap: 'wrap'
										...
										$("<input>", {type: 'number', min: 9, id: 'imageIndexInput', class: 'bg-transparent border-0 text-light text-right'}).css({
											flex: '1 1 auto',
											width: 0
										}).val(_self.currentImageIndex).on("change", function() {
						...
						$("<li>").append(
								$("<div>", {class: 'btn-group btn-group-sm btn-block bg-dark'}).append(
										$("<button>", {class: 'btn btn-flay py-0 text-capitalize', id: 'prev', type: 'button'}).html('prev').on("click", function() {_self.prev();}),
										$("<button>", {class: 'btn btn-flay py-0 text-capitalize', id: 'play', type: 'button'}).html('play').on("click", function() {_self.togglePlay();}),
										$("<button>", {class: 'btn btn-flay py-0 text-capitalize', id: 'next', type: 'button'}).html('next').on("click", function() {_self.next();})
								)
						),
				)
		).appendTo($("body"));
	...
	// add event listener
	$(this.settings.frameId).navActive(false).navEvent(function(signal, e) {
...
FlayImageEngine.prototype.playIntervalUp = function() {
	...
	_self.settings.showControlBox && $("#playInterval").val(_self.playIntervalTime);
	LocalStorageItem.set(_self.settings.storagePrefix + ".playIntervalTime", _self.playIntervalTime);
	if (_self.isPlay) {
		_self.stop();
		_self.play();
...
FlayImageEngine.prototype.toggleNextRandom = function() {
	...
	_self.settings.showControlBox && $("#random").prop('checked', _self.nextRandom);
	LocalStorageItem.set(_self.settings.storagePrefix + ".nextRandom", _self.nextRandom);
};
...
FlayImageEngine.prototype.togglePlay = function(isPlay) {
	...
	if (!_self.isPlay) {
		_self.play();
		_self.settings.showControlBox && $("#play").html("stop");
	} else {
		_self.stop();
		_self.settings.showControlBox && $("#play").html("play");
	}
};
