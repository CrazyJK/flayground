# flay-ai/packages/indexer — 지침

인덱서 파이프라인(ETL · 번역 · 캡션 · 임베딩 · 얼굴 · OCR). CLI 진입점 `python -m packages.indexer.cli <cmd>` (typer). 원본 `K:\Crazy\*` → SQLite + Qdrant 컬렉션.

## 파이프라인 단계

메타: `load → scan → history → fts` (= `all`).
AI: `translate → caption-posters → embed`(텍스트) · `embed-clip`(이미지) · `extract-faces → cluster-faces`(얼굴) · `ocr-posters`(OCR) · `sync-payload` · `cleanup`.
...
| 용도 | 모델 | 차원 / 비고 |
| --- | --- | --- |
| 텍스트 임베딩 | BGE-M3 (`BAAI/bge-m3`) | 1024d, `videos`/`poster_ocr`/`poster_caption` |
| 이미지 임베딩 | OpenCLIP `ViT-L-14` (`laion2b_s32b_b82k`) | 768d, `posters_clip` |
| 얼굴 | InsightFace `buffalo_l` | 512d, `faces` |
| OCR | RapidOCR (`rapidocr_onnxruntime`) | ONNX. PaddleOCR DLL 충돌 회피 |
| 번역 JP→KO | `facebook/nllb-200-distilled-600M` | `src_lang=jpn_Jpan`, `forced_bos_token_id`→`kor_Hang` |
| 캡션 | `config.yaml.models.vision` (Ollama VLM) | 배치 캡션 전용 |

`config.yaml.models.ocr_lang` 은 PaddleOCR 시절 레거시 값(현재 미사용) — [docs/flay-ai/TODO.md](../../../docs/flay-ai/TODO.md).

## 규칙

- **임베딩 모델/Qdrant 클라이언트는 `embed_text.py` 의 `_embedder()` / `_qdrant()` 를 재사용**한다(모듈 전역 lazy singleton). 모듈마다 새로 로드하면 VRAM 낭비.
- 포인트 ID 는 `embed_text.opus_to_id(opus)` (opus SHA1 앞 8바이트). 컬렉션 공통이라 cross-reference 가능 — 직접 ID 를 만들지 말 것.
...
- GPU 12GB 한정: LLM·CLIP·InsightFace 동시 로드 금지. 야간 스크립트(`bin/ai/nightly_index.ps1`)가 단계 사이 unload 를 조정.
...
`cluster_faces.py` 는 이름과 달리 **HDBSCAN 이 아니라** mutual-kNN + Union-Find(GPU) 커스텀 구현이다. config 의 `hdbscan_*` 파라미터명·`cli` docstring 의 "HDBSCAN" 표현은 레거시 명칭.
