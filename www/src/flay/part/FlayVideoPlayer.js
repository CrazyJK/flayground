...
export default class FlayVideoPlayer extends HTMLElement {
  ...
  #addVideoEvent() {
    this.video.addEventListener('error', (e) => console.error(this.opus, `[${e.type}]`, e)); // 에러가 발생하여 리소스를 로드할 수 없는 시점에 발생합니다.
    ...
    this.video.addEventListener('loadstart', (e) => console.debug(this.opus, `[${e.type}]`)); // 브라우저가 리소스를 로드하기 시작하는 시점에 발생합니다.
    this.video.addEventListener('loadedmetadata', (e) => console.debug(this.opus, `[${e.type}]`)); // 메타데이터가 로드 된 시점에 발생합니다.
    this.video.addEventListener('loadeddata', (e) => {
      this.loaded = true;
      this.duration = this.video.duration;
      console.debug(this.opus, `[${e.type}]`, 'loaded', this.loaded, 'duration', toTime(this.duration));
    }); // 미디어의 첫번째 프레임이 로딩 완료된 시점에 발생합니다.

    this.video.addEventListener('canplay', (e) => console.debug(this.opus, `[${e.type}]`)); // User agent가 미디어를 재생 가능한 시점에 발생합니다. 다만 전체 미디어를 재생하기 위해서는 콘텐츠의 버퍼링이 더 필요할 수 있습니다.
    this.video.addEventListener('play', (e) => console.debug(this.opus, `[${e.type}]`)); // HTMLMediaElement.play() 메소드 호출이나 autoplay 속성에 의해 paused 프로퍼티가 true 에서 false로 전환되는 시점에 발생합니다.
    ...
    this.video.addEventListener('canplaythrough', (e) => console.debug(this.opus, `[${e.type}]`)); // 추가 버퍼링 없이 전체 미디어를 재생할 수 있는 시점에 발생합니다.
    ...
    this.video.addEventListener('seeked', (e) => {
      console.debug(this.opus, `[${e.type}]`, 'time', toTime(this.video.currentTime));
    }); // 미디어 시킹이 완료되는 시점에 발생합니다.
    this.video.addEventListener('waiting', (e) => {
      this.playing = false;
      console.debug(this.opus, `[${e.type}]`, 'playing', this.playing);
    }); // 일시적인 버퍼 부족으로 재생이 정지된 시점에 발생합니다.
    this.video.addEventListener('ended', (e) => console.debug(this.opus, `[${e.type}]`)); // 미디어가 끝까지 재생 완료 된 시점에 발생합니다.
  }
