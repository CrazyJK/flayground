...
function ProgressBar(selector, args) {
	...
	$(this.selector).addClass("progress rounded-0").css({
		height: settings.height,
		backgroundColor: settings.backgroundColor
	}).empty().append(this.$bar);
...
$(".monitor-size").on("click", function() {
	$("#width").val($(this).text()).trigger($.Event('keyup', {keyCode: 13}));
	// show();
});
...
$("#playBtn").on("click", function() {
	...
	$(this).data("isPlay", isPlay).html(isPlay ? 'STO<i>P</i>' : '<i>P</i>LAY');
});
