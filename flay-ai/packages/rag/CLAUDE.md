...
- `_extract_meta()` 로 year/month/min_rank/rank/min_likes/min_play/max_play/sort/kind/playable 를 정규식 추출해 `search_videos` args 에 주입(LLM 누락 방어). 평점 "N 이상"→`min_rank`, "랭크 N"/"별점 N"→`rank`, "좋아요/하트/찜 N"→`min_likes`, "재생 N 이상/이하"→`min_play`/`max_play`, "최근/마지막에 본"→`sort=recent`, "오래 안 본"→`sort=oldest`. studio/actress 는 query 로 semantic+FTS 매칭.
