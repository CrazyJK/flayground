...
Express 4 기반 REST API 서버. TypeScript(ESM)로 작성하며 `tsx`(개발) / `tsup`(빌드) 사용. `../frontend/{public,dist}` 를 정적 서빙하고 루트 `.cert/` 인증서로 `https://flay.kamoru.jk`(:443, HTTP/2) 에 기동한다.
...
├── index.ts          # Express 앱 진입점 (정적 경로·인증서 경로는 __dirname 상대)
├── domain/           # 공유 타입 정의 (프론트엔드 @domain/* 과 공유)
...
빌드 결과는 단일 `dist/index.js` 로 번들된다 — `__dirname` 기준 상대 경로를 쓸 때 `src/` 와 `dist/` 양쪽에서 맞는지 확인한다.
