 ...
 * @returns {number} min(포함)과 max(제외) 사이의 랜덤 부동소수점 숫자
 * @throws {Error} min이 max보다 크거나 같을 경우
 */
 ...
 * @param {number} min 최솟값. 포함
 * @param {number} max 최댓값. 제외
 * @returns {number} min(포함)과 max(제외) 사이의 랜덤 정수
 * @throws {Error} min이 max보다 크거나 같을 경우
 */
 ...
 * @returns {number} min과 max를 모두 포함하는 범위 내의 랜덤 정수
 * @throws {Error} min이 max보다 클 경우
 */
